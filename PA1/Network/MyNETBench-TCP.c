...
/* Network Benchmarking using TCP*/
...
struct hostent *hp;
char output[100] = "./output/network-"; //network-TCP-1-1thread.out.dat
...
void *TCP_Client(void *arg) {
	...
	for (i=0;i<numthreads;i++)
	{
		...
		pthread_create( &clientthreads[i], NULL,  TCP_Client_Data , &pclieninfo[i]);
...
void *TCP_Server() {
	...
	//printf("TCP server thread memset.\n");
	...
	{		
		...
		ptinfo[i].sockid = clientfd;
		ptinfo[i].tid=i+1;
		printf("server created data thread:%d\n",i+1);
		pthread_create(&serverthreads[i],NULL,TCP_server_Data,&ptinfo[i]);
		  ...
		  // while(i<2)
		   //{
			   if(argc==2){
				   ...
				   pthread_create(&ithread[0], NULL, TCP_Server,NULL); //Server parent thread	
				   	   
			   }
				else if(argc==3){
					sleep(10); 			
					printf("creating client parent thread.\n");
					pthread_create(&ithread[0], NULL, TCP_Client,NULL); //Client parent thread	
				}
